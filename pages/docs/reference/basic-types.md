...
`Boolean` 타입은  논리 값을 나타내고, *true*{: .keyword } 과 *false*{: .keyword } 같은 두가지의 값이 있습니다.
...
`s의 [I]`: 문자열의 원소는 인덱싱 연산으로 접근 가능한 문자입니다.
...
여러분은  함수 선행 공백을 제거 할 수 있습니다 . [`trimMargin()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/trim-margin.html) function:
